...
@testset "ASE interface" begin
    ...
    function gen_lj_like(cell)
        ...
        cf = EDDPotentials.CellFeature(elem, p2=[2, 6], p3=[], q3=[])
        chain = Chain(Dense(zeros(1, 3)))
        chain.layers[1].weight[1, 2:3] .= [-2.0, 1.0]
        chain.layers[1].bias .= 0

        model = EDDPotentials.ManualFluxBackPropInterface(chain)
    ...
    EDDPotentials.get_energy(calc)
    atoms, acalc = EDDPotentialsTools.get_ase_atoms_and_calculator(calc)
