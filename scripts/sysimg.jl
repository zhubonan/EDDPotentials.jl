...
using PackageCompiler: get_compiler_cmd, create_sysimg_object_file, create_pkg_context, check_packages_in_project
using PackageCompiler: create_fresh_base_sysimage, gather_stdlibs_project, ensurecompiled, tempname
using PackageCompiler: compile_c_init_julia, active_project, NATIVE_CPU_TARGET, create_sysimg_from_object_file
using Pkg
...
function setup_temp_project()
    ...
    Pkg.develop(path=joinpath(dir, "../../CellBase/"))
    Pkg.develop(path=joinpath(dir, "../"))
    Pkg.add("Revise")
    Pkg.add(PKGS)
    Pkg.instantiate()
end
...
function build(sysimage_path=joinpath(DEPOT_PATH[1], "eddpdev.so");kwargs...)
    setup_temp_project()
