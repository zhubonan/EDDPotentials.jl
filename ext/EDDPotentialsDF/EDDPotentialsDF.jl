module EDDPotentialsDF

using DataFrames
using CellBase
using CellBase: read_res_many
using EDDPotentials: read_shelx_record, extract_res, ShelxRecord
...
## Arguments
- `records`: a squence of `ShelxRecord` objects.
- `basic`: if `true`, only basic information will be included in the `DataFrame`.

"""
function EDDPotentials.as_dataframe(records; basic=false, sort_enthalpy=true)

    if records[1] isa AbstractString
        records = read_shelx_record(records)
    end
...
"""
function EDDPotentials.select_refine(frame; dist_func=evdist, dist_cutoff=0.001, top_n=10)
    labels = String[]
