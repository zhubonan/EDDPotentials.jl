using EDDPotential
using EDDPotential: ComputedRecord, get_e_above_hull, PhaseDiagram
using CellBase
using Test
@testset "Records" begin
    ...
    #%%
    simplex = phased.simplices[EDDPotential.find_simplex(phased, records[1])]
    coord = EDDPotential.get_coord(records[1], phased.elements)
    bcord = EDDPotential.bary_coords(simplex, coord)
    @test allclose(EDDPotential.coords_from_bary(simplex, bcord), coord)
    @test allclose(bcord, [0, 1]; atol=1e-14)

    bcord = EDDPotential.bary_coords(simplex, records[2], phased)
    @test allclose(bcord, [1, 0]; atol=1e-14)

    @test EDDPotential.contains_point(simplex, coord)
