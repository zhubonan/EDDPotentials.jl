...
@testset "Records" begin
    ...
    @test get_e_above_hull(phased, ComputedRecord(Composition(:H2O), 1)) ≈ 2/3 atol=1e-14
    @test get_e_above_hull(phased, ComputedRecord(Composition(:H), 1)) ≈ 1. atol=1e-14
    @test get_e_above_hull(phased, ComputedRecord(Composition(:O), 0)) ≈ 0. atol=1e-14
end