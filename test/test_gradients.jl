...
@testset "Force buffer" begin
    ...
    fvec = vcat(EDDP.one_body_vectors(cell, cf), EDDP.feature_vector2(cf.two_body, cell), EDDP.feature_vector3(cf.three_body, cell))
    ...
    @test allclose(vec(jac), vec(permutedims(stotv[:, :, 1+n1bd:end, :], [3,4,1,2])), atol=1e-3)
    ...
    p0 = cell.positions
    od = NLSolversBase.OnceDifferentiable( x -> core_forces(cell, x, cf, n1bd), p0, core_forces(cell, p0, cf, n1bd); 
                                          inplace=false)
    grad = reshape(NLSolversBase.gradient!(od, p0), 3, length(cell))
    @test allclose(-fcore, grad;atol=1e-4)

    s0 = zeros(9)
    od = NLSolversBase.OnceDifferentiable( x -> core_stress(cell, x, cf, n1bd), s0, core_stress(cell, s0, cf, n1bd); 
                                          inplace=false)
...
@testset "Gradients" begin
    ...
    sjac = reshape(NLSolversBase.jacobian!(od, s0),
                  sum(EDDP.feature_size(calc.cf)), length(get_cell(calc)), 3,3)
